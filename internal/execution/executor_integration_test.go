...
func TestExecutorIntegration_HyperliquidSubmit(t *testing.T) {
	...
	// 目标仓位：在现有基础上调整 0.05%
	minAmount, amountPrecision, err := fetchMarketConstraints(tradeClient, tradeSymbol)
	if err != nil {
	...
	desiredAmount = quantizeAmount(desiredAmount, amountPrecision)
	if desiredAmount <= 0 {
		t.Skip("量化后下单数量为 0，跳过测试")
	}
	if desiredAmount < minAmount {
		desiredAmount = minAmount
	}
	if amountPrecision == 0 && desiredAmount < 1 {
		t.Skipf("市场要求整数合约量，当前最小数量 %.2f < 1", desiredAmount)
	}
...
func fetchMarketConstraints(client *ccxt.Hyperliquid, symbol string) (float64, int, error) {
	...
	minVal, _ := amount["min"].(float64)

	precision := 8
	if precMap, ok := marketMap["precision"].(map[string]interface{}); ok {
		if amtPrec, ok := precMap["amount"].(float64); ok {
			precision = int(amtPrec)
		}
	}

	return minVal, precision, nil
}

func quantizeAmount(amount float64, precision int) float64 {
	if precision < 0 {
		precision = 0
	}
